...
export const sendGreat = extendType({
  type: 'Mutation',
  definition(t) {
    t.field('sendGreat', {
      type: 'Boolean',
      ...
      async resolve(_, args, ctx) {
        try {
          ...
          const transferAmountLamport = 0.3 * LAMPORTS_PER_SOL
          ...
          await ctx.prisma.post.update({
            ...
            data: {
              greatNum: { increment: 1 },
            },
          })
          return true
        } catch (error) {
          console.log(`sendGreat: ${error}`)
          throw new Error(`sendGreat: ${error}`)
        }
      },
    })
  },
