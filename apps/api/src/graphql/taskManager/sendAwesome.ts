...
export const sendAwesome = extendType({
  type: 'Mutation',
  definition(t) {
    t.field('sendAwesome', {
      type: 'Boolean',
      ...
      async resolve(_, args, ctx) {
        try {
          ...
          const transferAmountLamport = 0.5 * LAMPORTS_PER_SOL
          ...
          await ctx.prisma.post.update({
            ...
            data: {
              awesomeNum: { increment: 1 },
            },
          })
          return true
        } catch (error) {
          console.log(`sendAwesome: ${error}`)
          throw new Error(`sendAwesome: ${error}`)
        }
      },
    })
  },
